...
import io.sunc.mj.mvp.model.MusicPlaybackTrack;
import io.sunc.mj.permission.PermissionManager;
import io.sunc.mj.provider.MusicPlaybackState;
import io.sunc.mj.provider.RecentStore;
import io.sunc.mj.provider.SongPlayCount;
import io.sunc.mj.receiver.MediaButtonIntentReceiver;
...
public class MusicService extends Service {
    ...
    private void addToPlayList(final long[] list, int position, long sourceId, ListenerUtil.IdType sourceType) {
        ...
        if (mPlaylist.size() == 0) {
            if (D) Log.d(TAG, "addToPlayList= mPlaylist.size() == 0");
            closeCursor();
            notifyChange(META_CHANGED);
        }
    }
    ...
    private void openCurrentAndMaybeNext(final boolean openNext) {
        synchronized (this) {
            ...
            if (mPlaylist.size() == 0) {
                if (D) Log.d(TAG, "openCurrentAndMaybeNext: mPlaylist.size() == 0");
                return;
            }
            stop(false);
            ...
            String path = getPath();
            if (D) Log.d(TAG, "openCurrentAndMaybeNext: path = " + path);
            ...
            while (true) {
                if (mCursor != null
                        && (!fromNet ? openFile(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/" + mCursor.getLong(IDCOLIDX))
                        : openFile(getPath()))) { //如果成功读取当前曲目的信息,结束循环 设置player和fileToPlay
                    break;
                }
...
//                    mPlayPos = pos;
    ...
    private void updateMediaSession(final String what) {
        ...
        if (what.equals(PLAYSTATE_CHANGED) || what.equals(POSITION_CHANGED)) { //播放状态改变时
            ...
            String albumUrl = MusicPlayer.getCurrentAlbumImage();
            if(albumUrl == null || !albumUrl.startsWith("http")) {
                albumUrl = ListenerUtil.getAlbumArtUri(getAlbumId()).toString();
            }
            Bitmap albumArt = ImageLoader.getInstance().loadImageSync(albumUrl);
            if (albumArt != null) {
    ...
    private Notification buildNotification() {
        ...
        Bitmap artwork;
        artwork = ImageLoader.getInstance().loadImageSync(albumUrl);
    ...
    public boolean openFile(final String path) {
        ...
        synchronized (this) {
            ...
            if (mPlayer.isInitialized()) {
                if (D) Log.d(TAG, "openFile: is not Initialized");
                mOpenFailedCounter = 0;
                return true;
            }
    ...
    public String getPath() {
        synchronized (this) {
            ...
            String mp3andpic = mCursor.getString(mCursor.getColumnIndexOrThrow(AudioColumns.DATA));
            if(mp3andpic != null && mp3andpic.startsWith("http://")) {
                String[] splite = mp3andpic.split("XXX");
                return splite[0];
            }
    ...
    public String getAlbumImage() {
        synchronized (this) {
            ...
            String mp3andpic = mCursor.getString(mCursor.getColumnIndexOrThrow(AudioColumns.DATA));
            Log.e("sunc", "mp3andpic==" + mp3andpic);
            if(mp3andpic != null && mp3andpic.startsWith("http://")) {
                String[] splite = mp3andpic.split("XXX");
                Log.e("sunc", "split==" + splite[0] + "x" + splite[1]);
                return splite[1];
            }
            return mp3andpic;
        }
    ...
    private void clearPlayList() {
        String path = getPath();
        if (D) Log.d(TAG, "clearPlayList: path = " + path);
        if(path != null && (path.startsWith("http://") || path.startsWith("https://"))) {
            ListenerUtil.deleteTracks(this, getIds());
        }
        mPlaylist.clear();
    }
    ...
    //当前播放列表的ID
    public long[] getIds() {
        int songNum = mPlaylist.size();
        long[] ret = new long[songNum];
        for (int i = 0; i < songNum; i++) {
            ret[i] = mPlaylist.get(i).mId;
        }
        return ret;
    }
    ...
    private static final class MultiPlayer implements MediaPlayer.OnErrorListener,
        ...
        private boolean setDataSourceImpl(final MediaPlayer player, final String path) {
            try {
                ...
                if (D) Log.d(TAG, "IOException:" + todo.getMessage());
                return false;
            } catch (final IllegalArgumentException todo) {
                if (D) Log.d(TAG, "IllegalArgumentException:" + todo.getMessage());
                return false;
            }
    ...
    private static final class ServiceStub extends IListenerService.Stub {
        ...
        @Override
        public String getAlbumImage() throws RemoteException {
            return mService.get().getAlbumImage();
        }
