...
import io.sunc.mj.event.PlaylistUpdateEvent;
import io.sunc.mj.event.RecentlyPlayEvent;
import io.sunc.mj.mvp.model.Playlist;
import io.sunc.mj.provider.FavoriteSong;
import io.sunc.mj.provider.RecentStore;
import io.sunc.mj.provider.SongPlayCount;
import io.sunc.mj.ui.dialogs.CreatePlaylistDialog;
import rx.android.schedulers.AndroidSchedulers;
import rx.functions.Action1;
import rx.functions.Func1;

import static io.sunc.mj.ListenerApp.getContext;
...
public class ListenerUtil {
    ...
    public static void updateNetMusic(final Context context, RecommendDetailList.Track music) {
        ContentValues value = new ContentValues();
        String mp3andpic = music.getMp3_url() + "XXX" + music.getAl().getPicUrl();
        value.put(MediaStore.Audio.AudioColumns._ID, music.getId());
        value.put(MediaStore.Audio.AudioColumns.DATA, mp3andpic);
        value.put(MediaStore.Audio.AudioColumns.ALBUM, music.getName());
        value.put(MediaStore.Audio.AudioColumns.TITLE, music.getName());
        value.put(MediaStore.Audio.AudioColumns.ARTIST, music.getAr().get(0).getName());
    ...
    public static void insertNetMusic(final Context context, List<RecommendDetailList.Track> music) {
        int len = music.size();
        ...
        for (int i = 0; i < len; i++) {
            ...
            long id = music.get(i).getId();
            String mp3andpic = music.get(i).getMp3_url() + "XXX" + music.get(i).getAl().getPicUrl();
            values[i].put(MediaStore.Audio.AudioColumns._ID, id);
            values[i].put(MediaStore.Audio.AudioColumns.DATA, mp3andpic);
            values[i].put(MediaStore.Audio.AudioColumns.ALBUM, music.get(i).getName());
            values[i].put(MediaStore.Audio.AudioColumns.TITLE, music.get(i).getName());
            values[i].put(MediaStore.Audio.AudioColumns.ARTIST, music.get(i).getAr().get(0).getName());
        }
        context.getContentResolver().bulkInsert(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, values);
    }
