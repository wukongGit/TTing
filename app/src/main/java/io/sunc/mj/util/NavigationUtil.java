...
import io.sunc.mj.ui.activity.SettingActivity;
...
import static io.sunc.mj.util.ListenerUtil.isLollipop;
...
public class NavigationUtil {
    ...
    @TargetApi(21)
    public static void navigateToRecommend(Activity context, long albumID, String albumName, Pair<View, String> transitionViews, String cover) {
        FragmentTransaction transaction = ((AppCompatActivity) context).getSupportFragmentManager().beginTransaction();
        Fragment fragment;
        ...
        if (isLollipop() && transitionViews != null) {
            Transition changeImage = TransitionInflater.from(context).inflateTransition(R.transition.image_transform);
            transaction.addSharedElement(transitionViews.first, transitionViews.second);
            fragment = RecommendDetailFragment.newInstance(albumID, albumName, true, transitionViews.second, cover);
            fragment.setSharedElementEnterTransition(changeImage);
            fragment.setSharedElementReturnTransition(changeImage);
        } else {
            transaction.setCustomAnimations(R.anim.activity_fade_in,
                    R.anim.activity_fade_out, R.anim.activity_fade_in, R.anim.activity_fade_out);
            fragment = RecommendDetailFragment.newInstance(albumID, albumName, false, null, cover);
        }
        transaction.hide(((AppCompatActivity) context).getSupportFragmentManager().findFragmentById(R.id.fragment_container));
        transaction.add(R.id.fragment_container, fragment);
        transaction.addToBackStack(null).commit();
